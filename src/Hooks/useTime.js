export default function useTime(publishDate) {
  ...
  if (years > 0) {
    return `${publishedTime.getDate()}/${
      publishedTime.getMonth() + 1
    }/${publishedTime.getFullYear()}`;
  } else if (months > 0) {
    return ` ${months} ${months > 1 ? "شهور" : "شهر"}`;
  } else if (days > 0) {
    return ` ${days} ${days > 1 ? "ايام" : "يوم"}`;
  } else if (hours > 0) {
    return ` ${hours} ${hours > 1 ? "ساعات" : "ساعه"}`;
  } else if (minutes > 0) {
    return ` ${minutes} ${minutes > 1 ? "دقائق" : "دقيقه"}`;
  } else {
    return ` ${seconds} ${seconds !== 1 ? "ثواني" : "ثانية"}`;
  }
}
